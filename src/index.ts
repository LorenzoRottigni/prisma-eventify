...
export async function loadEventBus(config: EventifyConfig): Promise<BusHandler | null> {
  try {
    const busHandler = new BusHandler(config)
    if (!(await busHandler.subscribeConfigEvents()))
      throw new Error('An error occurred while trying to subscribe autogenerated events.')
    return busHandler
  } catch (err) {
    console.error(err)
    return null
